...
int main(int argc, char *argv[]) {
    ...
    struct epoll_event touched_events[OPEN_MAX + 1];  //监听事件节点数组

    int lfd = init_listen_socket(g_epfd, SERVER_PORT);
    event_set(&g_events[OPEN_MAX], lfd, accept_connection, &g_events[OPEN_MAX]);
    event_add(g_epfd, EPOLLIN, &g_events[OPEN_MAX]);
    ...
    while(1){
        ...
        //监听g_epfd_tree，将满足事件的事件添加到g_events数组中，若一秒钟都没有时间满足，返回0
        int readied_count = Epoll_wait(g_epfd, touched_events, OPEN_MAX + 1, 100);

        for(int i = 0; i < readied_count; i++){
            //使用自定义结构体指针my_event_t，接受联合体data中void* ptr成员，已经对齐
            auto my_event = (struct my_event_t*)touched_events[i].data.ptr;
            if((touched_events[i].events & EPOLLIN) && (my_event->events & EPOLLIN)){
                my_event->call_back(my_event->fd, touched_events[i].events, my_event->self);
                //一系列的，接受连接请求，把cfd放到epoll树上，并接受数据
            }
            if((touched_events[i].events & EPOLLOUT) && (my_event->events & EPOLLOUT)){
                my_event->call_back(my_event->fd, touched_events[i].events, my_event->self);
            }
        }
    }
...
int init_listen_socket(int epfd, u_short port) {
    ...
    int flag = fcntl(listen_fd, F_GETFL);    //set sock as unblock
    flag |= O_NONBLOCK;
    fcntl(listen_fd, F_SETFL, flag);

    int opt = 1;
    setsockopt(listen_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));  //reuse port
    ...
    Bind(listen_fd, (struct sockaddr *) &server_sock_addr, sizeof(server_sock_addr));
    Listen(listen_fd, OPEN_MAX+1);
...
void receive_data(int fd, int events, void *my_event_s_arg) {
    ...
    int len = recv(fd, my_event->buf, sizeof(my_event->buf), 0);    //读fd，存入数据
